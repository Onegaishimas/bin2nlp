...
from src.models.analysis.results import AnalysisResult, SecurityFindings, FunctionInfo
...
from src.analysis.processors.string_extractor import StringExtractor, StringExtractionResult, StringExtractionConfig
...
class BinaryAnalysisEngine:
    ...
    async def _execute_format_detection(self, result: AggregatedAnalysisResult) -> None:
        """Execute format detection phase."""
        start_time = time.time()
        
        try:
            format_result = await self.format_detector.detect_format(result.file_path)
    ...
    async def _execute_function_analysis(self, result: AggregatedAnalysisResult) -> None:
        ...
        try:
            ...
            analysis_config = self._create_function_config()
            function_result = await self.function_analyzer.analyze_functions(
                result.file_path, 
                analysis_config
            )
    ...
    async def _execute_security_scanning(self, result: AggregatedAnalysisResult) -> None:
        ...
        try:
            # Configure based on focus areas
            scan_config = self._create_security_config()
            security_result = await self.security_scanner.scan_binary(
                result.file_path,
                scan_config
            )
    ...
    async def _execute_string_extraction(self, result: AggregatedAnalysisResult) -> None:
        """Execute string extraction phase."""
        start_time = time.time()
        
        try:
            # Configure based on analysis depth
            string_config = self._create_string_config()
    ...
    def _create_string_config(self) -> StringExtractionConfig:
        """Create string extraction configuration based on engine settings."""
        if self.config.analysis_depth == AnalysisDepth.QUICK:
    ...
    def _calculate_string_confidence(self, string_result: StringExtractionResult) -> float:
        """Calculate confidence score for string extraction results."""
        if string_result.total_strings_found == 0:
            return 0.0
        
        # Base confidence on extraction success
        base_confidence = 0.6
        
        # Factor in categorization success
        categorized_ratio = len(string_result.filtered_strings) / max(string_result.total_strings_found, 1)
        base_confidence += categorized_ratio * 0.3
        
        # Factor in analysis time (faster is better for strings)
        if string_result.analysis_time_ms < 30000:  # Under 30 seconds
            base_confidence += 0.1
    ...
    def _assess_risk_level(self, result: AggregatedAnalysisResult) -> str:
        ...
        if result.string_analysis:
            suspicious_categories = {"credential", "crypto_key", "url", "ip_address"}
            suspicious_count = sum(
                result.string_analysis.categories_found.get(cat, 0) 
    ...
    def _extract_key_findings(self, result: AggregatedAnalysisResult) -> List[str]:
        ...
        if result.string_analysis:
            interesting_cats = {"credential", "crypto_key", "url"}
            for cat in interesting_cats:
                count = result.string_analysis.categories_found.get(cat, 0)
                if count > 0:
                    findings.append(f"Found {count} {cat.replace('_', ' ')} strings")
    ...
    def _generate_recommendations(self, result: AggregatedAnalysisResult) -> List[str]:
        ...
        if result.string_analysis:
            creds_found = result.string_analysis.categories_found.get("credential", 0)
            if creds_found > 0:
                recommendations.append("Investigate potential credential exposure")
            
            crypto_found = result.string_analysis.categories_found.get("crypto_key", 0)  
            if crypto_found > 0:
                recommendations.append("Verify cryptographic key usage and security")
